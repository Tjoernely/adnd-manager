  ...
  for (const grp of PROFICIENCY_GROUPS) {
    ...
    for (const p of (grp.profs || [])) {
      ...
      try {
        ...
        if (res.rows.length) {
          await db.query('INSERT INTO proficiency_aliases(prof_id,alias) VALUES($1,$2) ON CONFLICT DO NOTHING',
            [res.rows[0].id, p.id]);
          ins++;
        } else skip++;
...
async function scrapeWiki() {
  ...
  for (const line of html.split('\n')) {
    ...
    const cells = [...line.matchAll(/<td[^>]*>(.*?)<\/td>/gi)]
    ...
    try {
      ...
      if (res.rows.length) {
        await db.query('INSERT INTO proficiency_aliases(prof_id,alias) VALUES($1,$2) ON CONFLICT DO NOTHING',[res.rows[0].id,name]);
        newCount++;
      }
...
async function seedClassAccess() {
  ...
  console.log('\u2713 Faerdig. '+t.rows[0].n+' NWPs i DB.');
}

async function main() {
  console.log('=== import-proficiencies.mjs v3 ===');
  try {
