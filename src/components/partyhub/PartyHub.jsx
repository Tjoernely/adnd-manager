...
export default function PartyHub({ campaign, user, onBack, onNavigate }) {
  ...
  useEffect(() => {
    ...
    ]).then(([chars, maps, quests, encounters, npcs]) => {
      if (cancelled) return;
      setData({
...
function EncountersTab({ encounters, isDM, onOpenModule, sectionCard }) {
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
        <div style={{ fontSize: 11, color: C.textDim }}>{encounters.length} encounter{encounters.length !== 1 ? 's' : ''}</div>
